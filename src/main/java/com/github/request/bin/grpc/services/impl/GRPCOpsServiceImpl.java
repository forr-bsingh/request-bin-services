...
import com.github.request.bin.grpc.services.*;
import com.github.request.bin.repos.BinRepo;
import com.github.request.bin.repos.OpsRepo;
import com.github.request.bin.utils.GRPCUtils;
...
public class GRPCOpsServiceImpl extends GRPCOpsServiceGrpc.GRPCOpsServiceImplBase {
    ...
    @LogThis
    @Override
    public void unaryPost(OpsPostRequest request, StreamObserver<com.github.request.bin.grpc.services.OpsPostResponse> responseObserver) {
        try {
            if (StringUtils.isBlank(request.getBin()) || request.getPayloadMap().isEmpty()) {
                ...
                if (binRepo.exists(request.getBin())) {
                    ...
                    responseObserver.onNext(com.github.request.bin.grpc.services.OpsPostResponse.newBuilder().putAllPayload(GRPCUtils.mapToStruct(payload)).build());
                    responseObserver.onCompleted();
                } else {
                    responseObserver.onError(Status.NOT_FOUND
                            .withDescription(String.format("Bin not found by the name of: %s, Create bin first.",
